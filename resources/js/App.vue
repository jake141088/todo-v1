        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <div class="col-md-4">
              ...
              <input type="text" id="modal-status-editar" v-model="tarefaEditada.status" class="form-control" required />
            </div>
          </div>
        </div>
        <div class="modal-footer">
    ...
    <table v-if="tarefas.length >= 1" class="table table-striped table-hover">
      ...
      <tbody>
        <tr v-for="tarefa in tarefas" :key="tarefa.id">
          ...
          <td>{{ tarefa.status }}</td>
          <td>
            <button @click="abrirModalEditar(tarefa)" class="btn btn-warning btn-sm">Editar</button>
            <button @click="removerTarefa(tarefa.id)" class="btn btn-danger btn-sm">Remover</button>
          </td>
        </tr>
      </tbody>
    </table>
